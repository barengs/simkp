...
const TARegistration = () => {
    ...
    useEffect(() => {
        // Fetch current active TA and eligibility status on mount
        Promise.all([
            dispatch(fetchMyTA()),
    ...
    const handleSubmit = (e) => {
        ...
        if (!formData.judul_diajukan.trim()) {
            toast.error("Judul Tugas Akhir wajib diisi.");
            return;
        }
        if (formData.judul_diajukan.length < 10) {
            toast.error("Judul Tugas Akhir minimal 10 karakter.");
            return;
        }
        if (!formData.latar_belakang_singkat.trim()) {
            toast.error("Latar belakang singkat wajib diisi.");
            return;
        }
        if (formData.latar_belakang_singkat.length < 20) {
            toast.error("Latar belakang singkat minimal 50 karakter.");
            return;
        }

        dispatch(registerTA(formData));
    };

    // Helper for rendering TA status badge
    ...
    // 1) Student has an active TA registration (not rejected, or is waiting / approved)
    if (myTA) {
        return (
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-gray-900 font-semibold text-sm truncate">
                                                {myTA.pembimbing_1?.name || "Belum ditentukan"}
                                            </p>
                                    ...
                                    <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg border border-gray-100">
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-gray-900 font-semibold text-sm truncate">
                                                {myTA.pembimbing_2?.name || "Belum ditentukan"}
                                            </p>
                                    ...
                                    <div className="bg-emerald-50/20 border border-emerald-900/10 p-4 rounded-xl space-y-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-gray-800 truncate">
                                                {myTA.internship.company?.name || "Mitra Kerja Praktek"}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <BookOpen size={16} className="text-emerald-600 shrink-0" />
                                            <span className="text-xs text-gray-600 truncate">
                                                Tema: {myTA.internship.theme?.name || "-"}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-2">
    ...
    // 2) Student is not eligible (Did not finish KP yet or profile incomplete)
        ...
        return (
            <div className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden p-6 sm:p-10 text-center flex flex-col items-center">
                    ...
                    <div className="w-full max-w-xl mt-10 p-6 bg-gray-50 rounded-2xl border border-gray-100">
                        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest text-left mb-6">Alur Kelulusan Akademik</h4>
                        
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-6 relative">
                            {/* Connector Line */}
                            <div className="absolute top-4 left-10 right-10 h-0.5 bg-gray-200 hidden sm:block"></div>

                            {/* Step 1: KP Register & Ongoing */}
                            <div className="flex items-center sm:flex-col gap-4 sm:gap-2 z-10 w-full sm:w-auto">
                                <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 border border-blue-200 flex items-center justify-center font-bold text-xs shrink-0">
                                    1
                                </div>
                                <div className="text-left sm:text-center">
                                    <p className="text-xs font-bold text-gray-800">Kerja Praktek (KP)</p>
                                    <p className="text-[10px] text-gray-400">Pendaftaran & Kegiatan</p>
                                </div>
                            </div>

                            {/* Step 2: KP Finished */}
                            <div className="flex items-center sm:flex-col gap-4 sm:gap-2 z-10 w-full sm:w-auto">
                                <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-600 border border-amber-200 flex items-center justify-center font-bold text-xs shrink-0 animate-pulse">
                                    2
                                </div>
                                <div className="text-left sm:text-center">
                                    <p className="text-xs font-bold text-gray-800">Evaluasi & Lulus KP</p>
                                    <p className="text-[10px] text-amber-600 font-semibold uppercase">Tahap Anda Saat Ini</p>
                                </div>
                            </div>

                            {/* Step 3: TA Registration */}
                            <div className="flex items-center sm:flex-col gap-4 sm:gap-2 z-10 w-full sm:w-auto">
                                <div className="w-8 h-8 rounded-full bg-gray-200 text-gray-400 flex items-center justify-center font-bold text-xs shrink-0">
                                    3
                                </div>
                                <div className="text-left sm:text-center">
                                    <p className="text-xs font-bold text-gray-400">Pendaftaran TA</p>
                                    <p className="text-[10px] text-gray-400">Pengajuan Judul TA</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    // 3) Student is eligible and has no active TA -> Render registration form!
    return (
        <div className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div className="mb-8 text-center sm:text-left">
                <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight flex items-center gap-3">
                ...
                <p className="mt-2 text-gray-500 text-sm">Ajukan judul proposal Tugas Akhir Anda berdasarkan data Kerja Praktek (KP) sebelumnya.</p>
            </div>

            {/* KP Reference Card (Summary of what we pull automatically) */}
            {eligibility?.internship && (
                <div className="bg-emerald-50/50 border border-emerald-200 rounded-2xl p-6 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-sm">
                    <div className="flex items-start gap-4">
                        <div className="w-12 h-12 bg-emerald-600 text-white rounded-xl flex items-center justify-center shrink-0 shadow-lg shadow-emerald-200 mt-0.5">
                            <CheckCircle size={24} />
                        </div>
                        <div>
                            <h3 className="text-lg font-bold text-emerald-950">Lulus Kerja Praktek!</h3>
                            <p className="text-sm text-emerald-800 mt-1">Sistem mendeteksi kelulusan KP Anda. Pengajuan TA akan secara otomatis ditautkan dengan data berikut:</p>
                            
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-2 mt-4 text-xs font-semibold text-emerald-900/80">
                                <div className="flex items-center gap-2">
                                    <Building size={14} className="text-emerald-600" />
                                    <span>{eligibility.internship.company?.name || "Manual / Mitra"}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <BookOpen size={14} className="text-emerald-600" />
                                    <span>Tema: {eligibility.internship.theme?.name || "-"}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Calendar size={14} className="text-emerald-600" />
                                    <span>Periode: {eligibility.internship.period ? `${eligibility.internship.period.semester} ${eligibility.internship.period.academic_year}` : "-"}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 sm:p-10 space-y-6">
                        <div>
                            ...
                            <p className="text-xs text-gray-400">Formulir sederhana tanpa unggahan berkas & pemilihan pembimbing.</p>
                        </div>

                        {/* Judul TA */}
                        <div className="space-y-2">
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest">Judul Tugas Akhir yang Diajukan</label>
                            <input
                                type="text"
                                name="judul_diajukan"
                                value={formData.judul_diajukan}
                                ...
                                placeholder="Masukkan rencana judul Tugas Akhir Anda..."
                            />
                            <p className="text-[10px] text-gray-400">Minimal 10 karakter.</p>
                        </div>

                        {/* Latar Belakang Singkat */}
                        <div className="space-y-2">
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest">Latar Belakang Singkat / Deskripsi Ide</label>
                            <textarea
                                name="latar_belakang_singkat"
                                value={formData.latar_belakang_singkat}
                                onChange={handleInputChange}
                                rows={6}
                                className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all text-sm font-medium text-gray-800 placeholder-gray-400"
                                placeholder="Deskripsikan secara singkat latar belakang masalah dan usulan solusi yang akan Anda kerjakan pada Tugas Akhir..."
                            />
                            <p className="text-[10px] text-gray-400">Minimal 50 karakter.</p>
                        </div>
                    </div>
